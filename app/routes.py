...
from app.forms import RegistrationForm, LoginForm, EditProfileForm, RegistrationForm, EmptyForm, PostForm, ResetPasswordRequestForm
...
def register():
	...
	if form.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()
		flash('Selamat pendaftaran berhasil!')
		return redirect(url_for('login'))
...
def edit_profile():
	...
	if form.validate_on_submit():
		...
		db.session.commit()
		flash('Pembaharuan anda telah di simpan.')
...
def follow(username):
	...
	if form.validate_on_submit():
		...
		if user == current_user:
			flash('Kamu tidak bisa mengikuti dirimu')
			return redirect(url_for('user',username=username))
		current_user.follow(user)
		db.session.commit()
		flash('Kamu mengikuti {}!'.format(username))
		return redirect(url_for('user', username=username))
	else:
		return redirect(url_for('index'))
...
def unfollow(username):
	...
	if form.validate_on_submit():
		...
		if user is None:
			...
			return redirect(url_for('index'))
		if user == current_user:
			flash('Kamu tidak bisa mengikuti dirimu sendiri!')
			return redirect(url_for('user', username=username))
		current_user.unfollow(user)
		db.session.commit()
		flash('Kamu berhenti mengikuti {}'.format(username))
		return redirect(url_for('user',username=username))
	else:
		return redirect(url_for('index'))
...
def reset_password_request():
	...
	if form.validate_on_submit():
		...
		flash('Periksa email anda untuk arahan mereset password')
		return redirect(url_for('login'))
